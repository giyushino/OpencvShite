...
if tf.__version__ >= "2.0.0":
    ...
    print("TensorFlow 2.x is not installed.")
...
def class_text_to_int(row_label):
    if row_label == 'Backward':
        return 1
    elif row_label == 'Forward':
        return 2
    elif row_label == 'Down':
        return 3
    elif row_label == 'Up':
        ...
        print(f"Warning: Label {row_label} not found.")
        return None
...
def create_tf_example(group, path):
    with tf_v1.gfile.GFile(os.path.join(path, '{}'.format(group.filename)), 'rb') as fid:
    ...
    tf_example = tf_v1.train.Example(features=tf_v1.train.Features(feature={
...
def main(_):
    writer = tf_v1.python_io.TFRecordWriter(FLAGS.output_path)
